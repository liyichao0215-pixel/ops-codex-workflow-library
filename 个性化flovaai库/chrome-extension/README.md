...
这是“Flova 全 Skill 搜索”的 v0.3 本地 Chrome 增强插件。旧版目录保留不动，v0.3 在“主 Skill + 辅助 Skill”基础上新增资产库桥接。
...
资产桥接会优先插入 Flova 可解析的原生资产引用。如果资产列表或详情里没有拿到 `resource_id`，插件会降级插入“未形成 Flova 原生引用”的提示，提醒你发送前用 Flova 原生 `@` 再确认一次。
...
- 搜索角色、场景、故事板、音色等资产。
- 点“选资产”：按整张资产卡加入本次引用。
- 点“插入到对话框”：插件会把主 Skill、资产引用和辅助提示一起放入输入框。
