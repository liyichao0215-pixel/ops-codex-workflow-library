...
这是“Flova 全 Skill 搜索”的 v0.4.2 本地 Chrome 增强插件。代码从 v0.2.1 的“主 Skill + 辅助 Skill”稳定版重新开始，资产功能改为“Flova 原生 @ 代理”，并新增驾驶舱 `flova-dispatch-plan` 导入入口。
...
- 粘贴矩阵号驾驶舱的 `flova-dispatch-plan`，自动预选主 Skill、辅助 Skill、资产关键词和正文提示词
...
驾驶舱执行包只用于预填选择和正文提示词。插件不读取本地队列、不自动发送、不自动生成，也不扣积分。
...
- 点“导入执行包”：粘贴驾驶舱输出的 `flova-dispatch-plan` 代码块，插件会按包里的主 Skill、辅助 Skill、资产关键词和正文提示词预填。
...
以后进入任何 Flova 项目页，只要 Chrome 启用了这个 v0.4.2 扩展，就可以点击右下角的“Skill / 原生 @”使用。
